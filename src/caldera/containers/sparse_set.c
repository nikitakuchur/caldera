...
void sparse_set_init(sparse_set* sparse_set, const size_t element_size) {
    for (int i = 0; i < SPARSE_MAX_SIZE; i++) {
...
void* sparse_set_add(sparse_set *sparse_set, const size_t id) {
...
void* sparse_set_get(const sparse_set *sparse_set, const size_t id) {
    const size_t index = sparse_set->sparse_array[id];
    if (index != NULL_INDEX) {
...
void sparse_set_remove(sparse_set *sparse_set, const size_t id) {
    const size_t dense_index = sparse_set->sparse_array[id];
